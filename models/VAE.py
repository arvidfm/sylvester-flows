...
class OrthogonalSylvesterVAENonAmortized(nn.Module):
    """
    Variational auto-encoder with orthogonal flows in the encoder.
    """

    def __init__(self, z_size, num_flows, num_ortho_vecs, cuda=False):
        super().__init__()
        ...
        # ~~Amortized~~Fixed flow parameters
        # Diagonal elements of R1 * R2 have to satisfy -1 < R1 * R2 for flow to be invertible
        self.diag_activation = nn.Tanh()

        self.full_d = nn.Parameter(self.FloatTensor(
            1, self.num_ortho_vecs,
            self.num_ortho_vecs, self.num_flows))
        self.diag1 = nn.Parameter(self.FloatTensor(
            1, self.num_ortho_vecs, self.num_flows))
        self.diag2 = nn.Parameter(self.FloatTensor(
            1, self.num_ortho_vecs, self.num_flows))

        self.q = nn.Parameter(self.FloatTensor(
            1, self.num_flows * self.z_size * self.num_ortho_vecs))
        self.b = nn.Parameter(self.FloatTensor(
            1, 1, self.num_ortho_vecs, self.num_flows))

        nn.init.kaiming_normal_(self.full_d)
        nn.init.normal_(self.diag1)
        nn.init.normal_(self.diag2)
    ...
    def forward(self, y):
        ...
        for k in range(self.num_flows):
            ...
            flow_k = getattr(self, 'flow_' + str(k))
            z, log_det_jacobian = flow_k(z, r1[:, :, :, k], r2[:, :, :, k],
                                         q_ortho[k, :, :, :], b[:, :, :, k])

            self.log_det_j += log_det_jacobian

        return z.view(y.shape), self.log_det_j.view(y.shape[:-1])
